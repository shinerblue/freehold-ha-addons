...
var HandshakeSchema = external_exports.object({
  ...
  // client MAC
  ap: external_exports.string().optional(),
  // AP MAC
  t: external_exports.string().optional(),
  // controller timestamp
  url: external_exports.string().optional(),
  // original destination URL
  ssid: external_exports.string().optional()
});
...
function parseHandshake(query) {
  const p = HandshakeSchema.parse(query);
  return {
    clientMac: normalizeMac(p.id),
    apMac: p.ap ? normalizeMac(p.ap) : null,
    timestamp: p.t ?? null,
    originalUrl: sanitizeRedirect(p.url ?? null),
    ssid: p.ssid ?? null
  };
}
...
function renderSplashHtml(ctx) {
  const b = ctx.branding ?? {};
  const name = esc(b.propertyName ?? "Guest WiFi");
  const color = b.primaryColor && /^#[0-9a-fA-F]{3,8}$/.test(b.primaryColor) ? b.primaryColor : "#2563eb";
  ...
  return [
    ...
    `<title>${name} \u2014 Connect to WiFi</title>`,
    "<style>",
    `:root{--brand:${color}}`,
    "body{font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;margin:0;background:#f5f5f7;color:#111;display:flex;min-height:100vh;align-items:center;justify-content:center}",
    ".card{background:#fff;max-width:420px;width:92%;padding:28px;border-radius:16px;box-shadow:0 8px 30px rgba(0,0,0,.08)}",
    ".logo{max-height:56px;margin-bottom:12px}",
    "h1{font-size:1.25rem;margin:0 0 4px}",
    "p.sub{color:#666;margin:0 0 18px;font-size:.92rem}",
...
async function handlePortalRequest(req, config2) {
  if (req.method === "GET") {
    ...
    const propertyId2 = config2.resolvePropertyId(hs.ssid);
    if (!propertyId2) {
  ...
  const body = req.body;
  const clientMac = normalizeMac(body["clientMac"] ?? body["id"] ?? "");
  ...
  const auth = await config2.authService.authorizeGuest({
    orgId: config2.orgId,
    propertyId,
    clientMac,
    durationMinutes: config2.authorizeMinutes ?? 1440,
    triggeredBy: "portal"
  });
  if (!auth.authorized) {
    return html(
      502,
      "<h1>Could not connect you to the internet</h1><p>Please try again, or contact your host.</p>"
    );
  }
  const dest = originalUrl ?? "/portal/success";
  return { status: 302, headers: { Location: dest }, body: "" };
}

// src/lib/guest-auth-service.ts
var import_node_https = require("node:https");
var UnifiControllerGuestAuth = class _UnifiControllerGuestAuth {
  constructor(config2) {
    this.config = config2;
  }
  vendor = "unifi";
  /** UniFi expects a colon-delimited lowercase MAC; we store it normalized. */
  static toColonMac(mac) {
    const hex = mac.replace(/[^0-9a-fA-F]/g, "").toLowerCase();
    return hex.length === 12 ? hex.match(/.{2}/g).join(":") : mac;
  }
  /** One HTTPS request against the controller; resolves status/headers/body. */
  request(urlStr, init) {
    const u = new URL(urlStr);
    return new Promise((resolve, reject) => {
      const r = (0, import_node_https.request)(
        {
          hostname: u.hostname,
          ...
          // Local UniFi gateways present a self-signed cert; skip verification
  ...
  async authorizeGuest(req) {
    const base = this.config.host.replace(/\/+$/, "");
    const site = this.config.site ?? "default";
    const fail = (reason) => ({
      authorized: false,
      controllerVendor: this.vendor,
    ...
    try {
      ...
      if (!ok(login.status)) return fail(`login_failed: HTTP ${login.status}`);
      ...
      const csrf = login.headers["x-updated-csrf-token"] ?? login.headers["x-csrf-token"] ?? "";
      if (!cookie) return fail("login_failed: no session cookie returned");
      const auth = await this.request(`${base}/proxy/network/api/s/${site}/cmd/stamgr`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          cmd: "authorize-guest",
          mac: _UnifiControllerGuestAuth.toColonMac(req.clientMac),
          minutes: req.durationMinutes
        })
      });
      if (!ok(auth.status)) return fail(`authorize_failed: HTTP ${auth.status}`);
      try {
        const parsed = JSON.parse(auth.body);
        if (parsed.meta?.rc && parsed.meta.rc !== "ok") {
          return fail(`authorize_failed: ${parsed.meta.msg ?? parsed.meta.rc}`);
        }
      } catch {
      }
      return { authorized: true, controllerVendor: this.vendor, failureReason: null };
    } catch (err) {
      return fail(`controller_error: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
};
var MockGuestAuthService = class {
  vendor = "mock";
  authorized = [];
  failNext = false;
  async authorizeGuest(req) {
    if (this.failNext) {
      this.failNext = false;
      return { authorized: false, controllerVendor: this.vendor, failureReason: "mock_fail" };
    }
    this.authorized.push(req);
    return { authorized: true, controllerVendor: this.vendor, failureReason: null };
  }
};
function getGuestAuthService(settings) {
  const vendor = settings.portal_controller_vendor ?? "unifi";
  switch (vendor) {
    case "unifi": {
      const cfg = settings.portal_controller;
      if (!cfg) throw new Error("portal_controller config required for unifi vendor");
      return new UnifiControllerGuestAuth(cfg);
    }
    case "mock":
      return new MockGuestAuthService();
    default:
      throw new Error(`Unknown portal controller vendor: ${vendor}`);
  }
}
...
function readEnvConfig() {
  const ssidMap = JSON.parse(process.env["PORTAL_SSID_MAP"] ?? "{}");
  const defaultPropertyId = process.env["PORTAL_DEFAULT_PROPERTY_ID"] ?? null;
  let branding;
  if (process.env["PORTAL_BRANDING"]) {
    branding = JSON.parse(process.env["PORTAL_BRANDING"]);
  }
  const authService = getGuestAuthService({
    portal_controller_vendor: process.env["PORTAL_CONTROLLER_VENDOR"] ?? "unifi",
    portal_controller: {
...
var SUCCESS_HTML = [
  ...
  "align-items:center;justify-content:center;background:#f5f5f7;color:#111}",
  ".c{text-align:center;max-width:380px;padding:24px}</style>",
  '<div class="c"><h1>You\u2019re connected \u2713</h1>',
  "<p>Enjoy your stay. You can close this page.</p></div>"
].join("");
async function handle(req, res, config2) {
  const url = new import_node_url.URL(req.url ?? "/", "http://localhost");
  ...
  if (req.method === "GET" && path === "/portal/success") {
  ...
  if (req.method === "POST") {
    ...
    portalReq = { method: "POST", query: {}, body };
  } else {
    const query = {};
    for (const [k, v] of url.searchParams) query[k] = v;
    portalReq = { method: "GET", query, body: {} };
  }
  const result = await handlePortalRequest(portalReq, config2);
